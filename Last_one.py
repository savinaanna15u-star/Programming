...
# Строим гистограмму с 15 столбцами и черными границами
...
# (вероятно, они не проходили тестирование)
...
plt.figure(figsize=(15, 6))  # Задаем размер графика
# Строим столбцы голубого цвета
plt.bar(short_names, values, color='skyblue')

# Настраиваем внешний вид диаграммы
plt.title('Рейтинг школ города по результатам тестирования')  # Заголовок
plt.xlabel('Школы')  # Подпись оси X
plt.ylabel('Баллы')  # Подпись оси Y
plt.xticks(rotation=60)  # Поворачиваем подписи школ на 60 градусов
plt.grid(axis='y')  # Включаем сетку по оси Y

plt.tight_layout()  # Автоматически настраиваем отступы
plt.show()  # Показываем финальную диаграмму