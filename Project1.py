# Импортируем нужные библиотеки (инструменты для работы)
...
# Оставляем только нужные столбцы (A-G и M-P)
columns_to_keep = [
    'Фамилия', 'Имя', 'Вариант', 'Адрес электронной почты',
    'ОУ', 'Состояние', 'Затраченное время',  # Это столбцы A-G
    'В. 5 /1,00', 'В. 6 /2,00', 'В. 7 /1,00', 'В. 8 /1,00'  # Это столбцы M-P
]
...
# Считаем общий балл за все задания
...
school_stats = df.groupby('Школа').agg({
    ...
    'Фамилия': 'count'  # еще один способ посчитать количество
}).round(2)

# Упрощаем названия столбцов
school_stats.columns = ['Средний_балл', 'Количество_учащихся', 'Всего_тестов']
school_stats = school_stats[['Средний_балл', 'Количество_учащихся']]
...
plt.bar(short_names, top_schools['Средний_балл'], color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd'])
...
# Строим гистограмму
plt.hist(df['Общий_балл'], bins=8, color='lightblue', edgecolor='black', alpha=0.7)
plt.title('Распределение баллов учащихся', fontsize=14, fontweight='bold')
plt.xlabel('Общий балл за задания M-P')
...
total_schools = df['Школа'].nunique()
...
print(f"• Лучшая школа: {school_stats.index[0]} - {school_stats.iloc[0]['Средний_балл']:.2f} балла")
